// In dev: use '' so requests go to same origin → Vite proxy forwards /api to backend (visible in Network tab)
// In prod or when VITE_API_URL is set: use that base URL
...
/** GET /api/health — backend status (visible in Network inspector as GET) */
export async function getHealth(): Promise<{ ok: boolean; db: boolean } | null> {
  try {
